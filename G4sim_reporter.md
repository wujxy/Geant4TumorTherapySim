...
图 11 展示了 uniform 和 shell 两种模式下的 y 投影细胞剂量热点图。每列上方为微区几何投影，颜色表示同一 `(x,z)` 柱内所有 y 层细胞剂量之和；中间柱状图给出肿瘤细胞与正常细胞的平均整细胞剂量；底部柱状图给出肿瘤细胞核与正常细胞核的平均剂量。

![图 11 Q2 uniform 与 shell 模式下的微区 y 投影剂量热点图](figures/Q2_micro_dose_map_uniform_vs_shell.png)

从图 11 可以看到，含 B10 肿瘤细胞柱比不含 B10 对照细胞柱更容易出现高剂量沉积，说明本模型中的微区选择性主要来自 B10 加载。图中的柱状图已给出两类细胞的平均整细胞剂量和平均细胞核剂量。

uniform 模式下，B10 可存在于细胞核附近及细胞核中，alpha/Li7 反应产物更容易在细胞核内沉积能量，因此肿瘤核平均剂量高于 shell 模式。shell 模式下，B10 位于细胞外侧壳层，短射程产物更多在壳层或细胞质附近沉积。本次 shell 运行中未观察到对照细胞核能量沉积，但该零值受有限统计量影响，不能单独解释为确定的保护效果。就提高肿瘤细胞核物理剂量而言，uniform 模式在本模型中表现更好。

### 4.5 B10 浓度扫描

为了研究“药物富集浓度”对 BNCT 反应活性和微区剂量的影响，本项目在固定中子能量和照射事件数下扫描 B10 浓度：
...
扫描结果显示，随着 B10 浓度提高，alpha+Li7 二次粒子产额总体增加，uniform 模式的肿瘤核剂量也总体高于 shell 模式。由于每个浓度点只进行一次有限统计运行，低浓度区的核剂量和局域化比例出现明显涨落，不能据此判断严格单调关系或最优浓度。因此，本扫描主要支持“提高 B10 浓度会增加 BNCT 反应活性”的趋势性结论。真实治疗还需考虑硼药物毒性和正常组织摄取，不能简单认为 B10 浓度越高越好。
...
由于 Q1 使用宏观人体 phantom 区域剂量，而 Q2 使用微观混合细胞 patch 的累计单细胞剂量，两者不能直接比较。为比较不同治疗机制在相同微区几何下的细胞选择性，本项目补充了一组 Q2 内部对照实验：保持同一个 `200 um x 200 um x 200 um` 混合细胞 patch、束流位置、`150 um` 束斑半径和初级粒子 histories 数，把中子 BNCT 与 `gamma 1 MeV`、`proton 45 MeV` 放到同一细胞尺度下比较。
...
图 15 使用同一 `x-z` 投影布局展示四种方法的整细胞剂量热点图。每个 panel 采用各自归一化色标，以便观察该方法内部剂量落点；图中角标给出对应的肿瘤细胞和正常细胞平均剂量。

![图 15 Q2 BNCT 与常规射线的同一细胞 patch 投影热点图](figures/Q2_therapy_comparison_projected_maps.png)

图 16 使用 `1 x 3` 柱状图汇总四种方法的整细胞指标：左图比较肿瘤细胞与对照细胞平均剂量，中图比较整细胞剂量局域化 `S_cell`，右图比较对照细胞负担 `D_normal_cell / D_cancer_cell`。跨射线主比较只采用整细胞口径，使结论集中在不同治疗机制对两类细胞的物理剂量选择性上；细胞核剂量则保留在 uniform 与 shell 的 B10 分布机制分析中。

![图 16 Q2 BNCT 与常规射线的整细胞指标对比](figures/Q2_therapy_comparison_summary.png)
...
3. B10 浓度升高时，alpha+Li7 二次粒子产额总体增加；低统计量下的核剂量比例存在明显涨落。
