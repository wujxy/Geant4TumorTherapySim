...
| 颈部 | `G4Tubs` | 半径 `50 mm`，高度 `90 mm` | `z = 295 mm` |
| 头部 | `G4Orb` | 半径 `90 mm` | `z = 430 mm` |
...
图 10 展示了 uniform 和 shell 两种模式下的 y 投影细胞剂量热点图。每个图上方为微区几何投影，颜色表示同一 `(x,z)` 柱内所有 y 层细胞剂量之和；下方嵌入柱状图，给出肿瘤细胞与正常细胞的平均细胞剂量。
...
uniform 模式下，B10 可以存在于细胞核附近甚至细胞核中，因此 alpha/Li7 反应产物更容易把能量沉积到细胞核内，肿瘤核平均剂量较高。shell 模式下，B10 位于外侧壳层，反应产物射程只有微米量级，更多能量沉积在壳层或细胞质附近，进入细胞核的概率降低，因此肿瘤核剂量低于 uniform 模式。
