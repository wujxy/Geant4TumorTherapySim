...
declare -a fluence_events=(2000 5000 10000 20000 50000 100000 200000)
...
for mode in "${b10_modes[@]}"; do
  for events in "${fluence_events[@]}"; do
    ...
    output="output_problem2_bnct_${mode}_fluence_${events}events.root"
    cat > "$macro" <<EOF
/control/verbose 0
/run/verbose 0
/event/verbose 0
...
/run/beamOn ${events}
EOF
    echo "Running neutron fluence scan ${mode} ${events} events"
    ./build/tumor_therapy "$macro" > "results/logs/problem2_bnct_${mode}_fluence_${events}events.log" 2>&1
  done
done

/usr/bin/python3 scripts/plot_assignment_results.py

echo "Generated figures/Q2_neutron_fluence_scan.png"
echo "Generated figures/Q2_neutron_fluence_projected_maps.png"
